...
import com.hungrymutt.form.SearchForm;
...
public final class AdvancedSearchAction extends Action {
	...
	throws Exception {
		 ...
		 * are saving data
		 */
		SearchForm curForm = (SearchForm) form;

		/* Validation is complete so we now perform the search */
		List results = RecipesPeer.advancedSearch(curForm.getCategory(),curForm.getKeyword(),curForm.getRating());
		ArrayList retVal = new ArrayList();
		Iterator it = results.iterator();
		
		// Build the html string for each search item so we can display to the
		// user the search we have made.
		String resultString = "Results for search where average rating > " + curForm.getRating();
		if (curForm.getKeyword().trim().length() > 0) {
			resultString += " and keyword = " + curForm.getKeyword();
		}
		if (curForm.getCategory().trim().length() > 0) {
			resultString += " and category = " + curForm.getCategory();
		}
