<jsp:directive.page import="com.sns.scout.managers.GroupsPeer"/>
<jsp:directive.page import="com.sns.scout.managers.Groups"/>
...
<!-- Now the main text -->
	...
	<tr><td><table border=1 bordercolor="GRAY" cellpadding=0 cellspacing=0 rules="none" width=90% id="report">
			...
			while (searchresults.hasNext()) {
				Groups curProject = (Groups) searchresults.next(); 
				if (odd) { %>
					<pg:item><tr class="ODD"><td><a href="/Scout/GroupEdit.do?id=<%=curProject.getGroupid()%>">Edit</a></td><td><%=curProject.getDisplayname() %></td></tr></pg:item>
				<% } else { %>
					<pg:item><tr class="EVEN"><td><a href="/Scout/GroupEdit.do?id=<%=curProject.getGroupid()%>">Edit</a></td><td><%=curProject.getDisplayname() %></td></tr></pg:item>
				<% }
				odd=!odd;
			} %>
		</table></td></tr>
