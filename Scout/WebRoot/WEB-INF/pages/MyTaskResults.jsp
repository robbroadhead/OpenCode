...
<%
	...
	if (results == null) {
		results = TaskPeer.getMyTasks(Integer.parseInt(request.getSession().getAttribute("scout.Userid").toString()));
...
<!-- Now the main text -->
<h2 align="center">My Open Tasks</h2>
...
<p align="center">
<table border=1 bordercolor="GRAY" cellpadding=0 cellspacing=0 rules="none" width=90% id="report">	
	<tr><th width=15%>Action</th><th width=20%>Project</th><th width=10%>Status</th><th width=55%>Summary</th></tr>
		<% if (offset.intValue()  + 25 > resultcount ) { 
			...
			while (searchresults.hasNext()) {
				...
				if (odd) { %>
					<pg:item><tr class="ODD"><td><a href="/Scout/TaskEdit.do?task=true&id=<%=curTask.getTaskid()%>">Edit</a></td><td><%=ProjectPeer.retrieveByPK(curTask.getProjectid()).getName()%></td><td><%=LkpstatusPeer.retrieveByPK(curTask.getStatusid()).getName() %></td><td><%=curTask.getSummary() %></td></tr></pg:item>
				<% } else { %>
					<pg:item><tr class="EVEN"><td><a href="/Scout/TaskEdit.do?task=true&id=<%=curTask.getTaskid()%>">Edit</a></td><td><%=ProjectPeer.retrieveByPK(curTask.getProjectid()).getName()%></td><td><%=LkpstatusPeer.retrieveByPK(curTask.getStatusid()).getName() %></td><td><%=curTask.getSummary() %></td></tr></pg:item>
				<% }
				odd=!odd;
			} 
		%>
