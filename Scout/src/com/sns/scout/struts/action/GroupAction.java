 ...
 * 
 * Description: This is the editing action code for a Group. 
 *
 */
public final class GroupAction extends Action {
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form,
		 ...
		 */
		GroupForm curForm = (GroupForm) form;
		Groups val = new Groups();

		/* Validation is complete so we now save the object */
		val.setGroupid(curForm.getId());
		...
		if (curForm.getDisplayname().trim().length() > 0) {
			...
			val.save();
			curForm.setId(val.getGroupid());
			System.out.println("Saved Group:" + val.getGroupid());
		}
