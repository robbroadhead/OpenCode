...
public final class LogoutAction extends Action {
	...
	throws Exception {
		...
		*/
		if (request.getSession().getAttribute("scout.Userid")!=null) {
			// If we had a timeout then we still want to log the logout action and may
			// need to re-initialize Torque.
			if (!Torque.isInit()) {
				try {
			...
			request.getSession().removeAttribute( "scout.User");
			request.getSession().removeAttribute( "scout.Userid");
			Torque.shutdown();
		}
