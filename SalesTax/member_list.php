...
include("include/member_variables.php");
	include("include/Tax_Summary_settings.php");
...
{	
	...
	if($lookuptable=="taxitem" && $lookupfield=="userID") 
	{
		$linkfield = "MemberID";
		$dispfield = "DisplayName";
			}
	if($lookuptable=="Tax_Summary" && $lookupfield=="userID") 
	{
		$linkfield = "MemberID";
		$dispfield = "DisplayName";
			}
...
//	this code must be executed after the inlineedit.js is loaded
	...
	$jscode_end.='
			window.inlineEditing'.$id.' = new InlineEditing(\'member\',\'php\','.$id.');
			'.$afteredited_handler;
...
"\nbLoading=false;\r\n";
	...
	$jscode.="MASTER_PREVIEW_TABLE='member_masterpreview.php';\r\n";
...
$body = array();
if($mode==LIST_SIMPLE)
	$html_begin.="<form name=\"frmSearch\" method=\"GET\" action=\"member_list.php\">";
else
{
	$html_begin.="<form name=\"frmSearch".$id."\" target=\"flyframe".$id."\" method=\"GET\" action=\"member_list.php\">";
...
//Session key fields for sort		
	...
	if($condition)
	{
		for($i=0;$i<$lenArr;$i++)
		{
			...
			{
				$href="member_list.php?orderby=a".GoodFieldName($order_field);
			}
			else
			{
				$href="member_list.php?orderby=".($order_dir=="a" ? "d" : "a").GoodFieldName($order_field);	
			}
			$orderlinkattrs="";
			if($mode==LIST_LOOKUP)
			{
...
if (@$_REQUEST["mdelete"])
{
	...
	{
		$keys=array();
		$keys["MemberID"]=refine($_REQUEST["mdelete1"][mdeleteIndex($ind)]);
		$selected_recs[]=$keys;
	}
}
...
if(no_output_done() && count($selected_recs) && $mode==LIST_SIMPLE){	
	...
	header("Location: member_".$pageName."?a=return");
...
//	add a key field to the select list
	...
	$gsqlHead.=",(".$countsql.") as Tax_Summary_cnt ";
	...
	if($maxpages>1)
	{
		...
		if($mode==LIST_SIMPLE)
			$jscode_end.="window.GotoPage = function(nPageNumber)
				{
					window.location='member_list.php?goto='+nPageNumber;
				};";
		else
			$jscode_end.="window.GotoPage".$id." = function (nPageNumber)
				{
					window.frames['flyframe".$id."'].location='member_list.php?".$lookupparams."&goto='+nPageNumber;
				};";
...
//	add grid data	
	...
	$editable=CheckSecurity($data["MemberID"],"Edit");
...
//	detail tables
	...
	$detailkeys=array();
	$detailkeys[]="userID";
	$masterquery="mastertable=member";
	...
	{			
		...
		$detailid[]=make_db_value($d,$data[$m],"","","Tax Summary");
	}
	...
	$securityClause = SecuritySQL("Search", "Tax Summary");	
	// add where 
	if (strlen($securityClause))
	...
	foreach($masterkeys as $idx=>$val)
	{
		if($masterwhere)
			$masterwhere.=" and ";
	$masterwhere.=GetFullFieldName($detailkeys[$idx],"Tax Summary")."=".$detailid[$idx];
	}
	$data["Tax_Summary_cnt"]=gSQLRowCount_int($sqlHead,$sqlFrom,$sqlWhere,$sqlTail,$masterwhere,1);
	}

//detail tables
	$record["Tax_Summary_dtable_link"]=$allow_Tax_Summary;
	$record["Tax_Summary_dtablelink_attrs"]="href=\"Tax_Summary_list.php?".$masterquery."\" id=\"master_Tax_Summary".$recid."\"";
	$dpreview=true;
		if($data["Tax_Summary_cnt"]+0)
		$record["Tax_Summary_childcount"]=true;
	else
		$dpreview=false;
	$record["Tax_Summary_childnumber"]=$data["Tax_Summary_cnt"];
		if(!($data["Tax_Summary_cnt"]+0))
	{
		$record["Tax_Summary_dtable_link"]=false;
		$dpreview=false;
	}
	if($dpreview)
	{
			$record["Tax_Summary_dtablelink_attrs"].=" onmouseover=\"RollDetailsLink.showPopup(this,'Tax_Summary_detailspreview.php'+this.href.substr(this.href.indexOf('?')));\" onmouseout=\"RollDetailsLink.hidePopup();\"";
		}
...
//	key fields
	...
	$copylink="";
	$keylink="";
		$keyblock.=rawurlencode($data["MemberID"]);
	$editlink.="editid1=".htmlspecialchars(rawurlencode($data["MemberID"]));
	$copylink.="copyid1=".htmlspecialchars(rawurlencode($data["MemberID"]));
	$keylink.="&key1=".htmlspecialchars(rawurlencode(@$data["MemberID"]));

	$record["editlink_attrs"]="href=\"member_edit.php?".$editlink."\" id=\"editlink".$recid."\"";
	$record["inlineeditlink_attrs"]= "href=\"member_edit.php?".$editlink."\" onclick=\"return inlineEditing".$id.".inlineEdit('".$recid."','".$editlink."');\" id=\"ieditlink".$recid."\"";
	$record["copylink_attrs"]="href=\"member_add.php?".$copylink."\" id=\"copylink".$recid."\"";
	$record["viewlink_attrs"]="href=\"member_view.php?".$editlink."\" id=\"viewlink".$recid."\"";
	if($mode!=LIST_LOOKUP)
	{
...
if($mode==LIST_SIMPLE)
{
	...
	$xt->assign("printlink_attrs","href=\"member_print.php\" onclick=\"window.open('member_print.php','wPrint');return false;\"");
	$xt->assign("printalllink_attrs","href=\"member_print.php?all=1\" onclick=\"window.open('member_print.php?all=1','wPrint');return false;\"");
	$xt->assign("export_link",$allow_export);
	$xt->assign("exportlink_attrs","href=\"member_export.php\" onclick=\"window.open('member_export.php','wExport');return false;\"");
	...
	if(!\$('input[@type=checkbox][@checked][@name^=selection]').length)
		return true;
	document.forms.frmAdmin.action='member_print.php';
	document.forms.frmAdmin.target='_blank';
	document.forms.frmAdmin.submit(); 
	document.forms.frmAdmin.action='member_list.php'; 
	document.forms.frmAdmin.target='_self';return false\"
	href=\"member_print.php\"");
	$xt->assign("exportselected_link",$allow_export);
	$xt->assign("exportselectedlink_attrs","disptype=\"control1\" onclick=\"
	if(!\$('input[@type=checkbox][@checked][@name^=selection]').length)
		return true;
	document.forms.frmAdmin.action='member_export.php';
	document.forms.frmAdmin.target='_blank';
	document.forms.frmAdmin.submit(); 
	document.forms.frmAdmin.action='member_list.php'; 
	document.forms.frmAdmin.target='_self';return false;\"
	href=\"member_export.php\"");
	
	$xt->assign("add_link",$allow_add);
	$xt->assign("copy_column",$allow_add);
	$xt->assign("addlink_attrs","href=\"member_add.php\" onClick=\"window.location.href='member_add.php'\"");
	$xt->assign("inlineadd_link",$allow_add);
	$xt->assign("inlineaddlink_attrs","href=\"member_add.php\" onclick=\"return inlineEditing".$id.".inlineAdd(flyid++,null,'member_add.php');\"");
...
for (i=0;i<frmAdmin.elements['selection[]'].length;++i) 
	...
	$xt->assign("editselectedlink_attrs","href=\"member_edit.php\" disptype=\"control1\" name=\"edit_selected".$id."\" onclick=\"\$('input[@type=checkbox][@checked][@id^=check]').each(function(i){
	...
	if($mode!=LIST_LOOKUP)
	{
 		$xt->assign("Tax_Summary_dtable_column",$allow_Tax_Summary);
	}
...
//	$xt->assign("checkbox_column",true);
	$xt->assign("inlineadd_link",$allow_add);
	$xt->assign("inlineaddlink_attrs","href=\"member_add.php\" onclick=\"return inlineEditing".$id.".inlineAdd(flyid++,".$id.",'member_add.php');\"");
//	$xt->assign("inlineedit_column",$allow_edit);
}
...
$xt->assign("asearchlink_attrs","href=\"member_search.php\" onclick=\"window.location.href='member_search.php';return false;\"");
$xt->assign("import_link",$allow_import);
$xt->assign("importlink_attrs","href=\"member_import.php\" onclick=\"window.location.href='member_import.php';return false;\"");
...
if($display_grid)
{
	if($mode==LIST_SIMPLE)
		...
		"<form method=\"POST\" action=\"member_list.php\" name=\"frmAdmin\" id=\"frmAdmin\">
		<input type=\"hidden\" id=\"a\" name=\"a\" value=\"delete\">",
		"</form>");
	elseif($mode==LIST_LOOKUP)
		$xt->assign_section("grid_block",
		"<form method=\"POST\" action=\"member_list.php\" name=\"frmAdmin".$id."\" id=\"frmAdmin".$id."\" target=\"flyframe".$id."\">
...
if($mode==LIST_SIMPLE)
{
	...
	$xt->assign("recordspp_attrs","onchange=\"javascript: document.location='member_list.php?pagesize='+this.options[this.selectedIndex].value;\"");
	$xt->assign("pages_block",$allow_search && $rowsfound);
}
else
	$xt->assign("recordspp_attrs","onchange=\"javascript: window.frames['flyframe".$id."'].location='member_list.php?".$lookupparams."&pagesize='+this.options[this.selectedIndex].value;\"");
