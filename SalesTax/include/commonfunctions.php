...
// long binary data?
	...
	else if($format==FORMAT_FILE)
	{
		...
		if($strTableName=="member")
		{
			if($data)
				$iquery.="&key1=".rawurlencode($data["MemberID"]);
		}
		if($strTableName=="taxitem")
		{
			if($data)
				$iquery.="&key1=".rawurlencode($data["itemID"]);
		}
		if($strTableName=="Tax Summary")
		{
		}
...
//	member
	if($table=="member")
	{
			$value=make_db_value("MemberID",$keys["MemberID"]);
				$valueisnull = ($value==="null");
		if($valueisnull)
			$strWhere.=GetFullFieldName("MemberID")." is null";
		else
			$strWhere.=GetFullFieldName("MemberID")."=".make_db_value("MemberID",$keys["MemberID"]);
	}

//	taxitem
	if($table=="taxitem")
	{
			$value=make_db_value("itemID",$keys["itemID"]);
				$valueisnull = ($value==="null");
		if($valueisnull)
			$strWhere.=GetFullFieldName("itemID")." is null";
		else
			$strWhere.=GetFullFieldName("itemID")."=".make_db_value("itemID",$keys["itemID"]);
	}

//	Tax Summary
	if($table=="Tax Summary")
	{
	}
	return $strWhere;
}
...
global $cAdvSecurityMethod, $strTableName;
	...
	if(@$_SESSION["AccessLevel"]!=ACCESS_LEVEL_ADMINGROUP && strpos($strPerm, "M")===false)
	{
		if($strTableName=="member")
		{
				if(!((string)$_SESSION["_".$strTableName."_OwnerID"]===(string)$strValue))
				return false;
		}
		if($strTableName=="taxitem")
		{
...
global $cAdvSecurityMethod,$strTableName;
	...
	if(@$_SESSION["AccessLevel"]!=ACCESS_LEVEL_ADMINGROUP)
	{
		...
		if($table=="member")
		{
				$ret=GetFullFieldName(GetTableOwnerID($table), $table)."=".make_db_value(GetTableOwnerID($table),$ownerid);
		}
		if($table=="taxitem")
		{
				$ret=GetFullFieldName(GetTableOwnerID($table), $table)."=".make_db_value(GetTableOwnerID($table),$ownerid);
...
//	build SQL strings for lookup wizards
		if($strTableName=="taxitem" && $field=="userID") 
		{
			...
			$LookupSQL = "select ";
						$LookupSQL .= "`member`.`MemberID`";
						
			$LookupSQL .= ",`member`.`DisplayName`";
						$LookupSQL .= " from `member` ";
			
									
			
						$LookupSQL.=" ORDER BY `member`.`DisplayName`";
					}
		if($strTableName=="Tax Summary" && $field=="userID") 
		{
			$strPerm = GetUserPermissions("member");
					$LinkField="MemberID";
			$DisplayField="DisplayName";
			...
			//	add new item
				if($mode!=MODE_SEARCH && $addnewitem || $lookuptype==LCT_LIST)
				{
					...
					if($addnewitem)
					{
						if(!$advancedadd)
						{
							echo "<a href=# onclick=\"window.open('member_addnewitem.php?".
...
//	get the initial value
			...
			{
								$lookup_SQL = "SELECT ";
								$lookup_SQL .= "`member`.`MemberID`";
								$lookup_SQL .= ",`member`.`DisplayName`";
				$lookup_SQL .= " FROM `member` ";
								$lookup_SQL .= " WHERE `member`.`MemberID`=".make_db_value($field,$value)."";
												$lookup_SQL.= " ORDER BY `member`.`DisplayName`";
								}
			if($strTableName=="Tax Summary" && $field=="userID") 
			{
								$lookup_SQL = "SELECT ";
...
function BuildEditControl($field , $value, $format, $edit, $secondfield=false, $id="")
{
	...
	if($strTableName=="member")
	{
		$keylink.="&key1=".rawurlencode(@$keys["MemberID"]);
		$iquery.=$keylink;
	}
	if($strTableName=="taxitem")
	{
		$keylink.="&key1=".rawurlencode(@$keys["itemID"]);
		$iquery.=$keylink;
	}
	if($strTableName=="Tax Summary")
	{
